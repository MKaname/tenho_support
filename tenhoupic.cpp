...
char *preset_file = "hai/sample.png";
int d;
int hai[38];//手牌
...
int main(int argc, char **argv){
  ...
  for(i=0;i<contours.size(); i++){
    ...
    if(fabs(minP.x-maxP.x)/fabs(minP.y-maxP.y)>8 && fabs(minP.x-maxP.x)/fabs(minP.y-maxP.y)<11 && fabs(minP.x-maxP.x)>size.width/3.0){
      //手牌部分の矩形を描く
      d = maxP.y - minP.y;
      rect.x = minP.x-BIAS;
      rect.y = minP.y-BIAS;
